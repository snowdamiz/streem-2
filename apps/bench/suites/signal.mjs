...
export async function run() {
  ...
  // @streeem/core — with owner scope (matches real-world usage)
  bench.add('@streeem/core signal (with createRoot)', () => {
    createRoot((dispose) => {
      const s = signal(0)
      s.set(1)
      const v = s()
  ...
  // @streeem/core — primitive only, no owner scope (apples-to-apples with Preact)
  // Valid: signals work without createRoot in production builds (no owner warnings in prod)
  bench.add('@streeem/core signal (primitive only)', () => {
    const s = signal(0)
    s.set(1)
    const v = s()
    return v
  ...
  // SolidJS — uses createRoot (same scope as @streeem/core with-root variant)
