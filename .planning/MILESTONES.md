...
**Key accomplishments:**
- Built DOM-free reactive signal primitives (/core): signal(), computed(), effect(), createRoot(), onCleanup() with zero DOM dependency, dev-mode reactive-context warnings, and 40-test Vitest coverage
- Shipped /dom: JSX factory, fine-grained DOM bindings (text/attr/class/style/events), Show/For/ErrorBoundary/Suspense components, component-runs-once model, and Vite HMR signal-state preservation
- Four stream adapters (/streams): WebSocket (auto-reconnect + exponential backoff), SSE, ReadableStream, Observable — each with onCleanup auto-close, typed status signal, and batch()/throttle()/debounce() backpressure
- TypeScript-typed Lit web component interop (/lit): prop:/attr:/on: prefix dispatch, CEM-driven IntrinsicElements type generation, Playwright-verified Shadow DOM event handling
...
- LIT-04 type augmentation not in dist/: /lit/dist/index.d.ts omits JSX IntrinsicElements for Shoelace elements (runtime unaffected; prop-safety for sl-* types falls back to catch-all)
...
**Key accomplishments:**
- Fixed TypeScript IntrinsicElements for all sl-* Shoelace elements in /lit dist/ (vite-plugin-dts beforeWriteFile hook pattern)
