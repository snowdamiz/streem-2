...
provides:
  - "@streem/dom package with ./jsx-runtime and ./jsx-dev-runtime exports for jsxImportSource resolution"
...
tech-stack:
  ...
  patterns:
    - "jsxImportSource: @streem/dom in tsconfig to route TSX compilation through package's own jsx-runtime"
...
key-decisions:
  - "Remove explicit rootDir from tsconfig.json: tests/ alongside src/ requires TypeScript to infer rootDir rather than constrain it — mirrors @streem/core pattern"
  - "@streem/core placed in dependencies (not devDependencies) in package.json since it is a runtime workspace package consumed by h() and render()"
...
**@streem/dom package with jsx-runtime entry point, real-DOM h() factory using createRoot scope for components, render() mount function, and JSX namespace types — all verified by 5 happy-dom tests**
...
- Scaffolded @streem/dom package with three package.json exports (`"."`, `"./jsx-runtime"`, `"./jsx-dev-runtime"`) so `jsxImportSource: "@streem/dom"` resolves correctly in TypeScript
...
- `packages/dom/package.json` - Package metadata with three exports and workspace:* @streem/core dep
- `packages/dom/tsconfig.json` - TypeScript config with jsx: react-jsx and jsxImportSource: @streem/dom
...
- `pnpm-lock.yaml` - Updated with happy-dom and @streem/core workspace link

## Decisions Made

- Removed `rootDir: "./src"` from tsconfig.json because including `tests/` alongside `src/` causes TS6059. TypeScript infers rootDir correctly without it, matching the @streem/core pattern.
- `@streem/core` placed in `dependencies` (not `devDependencies`) since it is a runtime import consumed by h() and render() in production code.
...
- **Fix:** Removed the `rootDir` compiler option from packages/dom/tsconfig.json. TypeScript infers rootDir from included directories, identical to how @streem/core is configured.
- **Files modified:** packages/dom/tsconfig.json
- **Verification:** `pnpm --filter @streem/dom exec tsc --noEmit` exits 0; all 5 tests still pass
- **Committed in:** 53fd19d (Task 2 commit)

---

**Total deviations:** 1 auto-fixed (1 bug — rootDir conflict)
**Impact on plan:** Auto-fix necessary for TypeScript compilation. No scope creep — follows established @streem/core pattern.
...
- @streem/dom package structure is complete and all success criteria are met
