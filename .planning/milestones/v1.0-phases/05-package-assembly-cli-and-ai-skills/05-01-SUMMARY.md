...
requires:
  - phase: 01-reactive-core
    provides: "@streem/core signals — signal, computed, effect, createRoot, onCleanup, getOwner, runWithOwner"
  - phase: 02-jsx-runtime-and-component-model
    provides: "@streem/dom — h, Fragment, render, onMount, Show, For, ErrorBoundary, Suspense, streemHMR, jsx-runtime subpath"
  - phase: 03-streaming-primitives
    provides: "@streem/streams — fromWebSocket, fromSSE, fromReadable, fromObservable, batch, throttle, debounce"
provides:
  ...
  - "packages/streem/dist/jsx-runtime.js — re-exports @streem/dom/jsx-runtime, enabling jsxImportSource: 'streem'"
  - "packages/streem/dist/jsx-dev-runtime.js — re-exports @streem/dom/jsx-dev-runtime for dev mode"
...
tech-stack:
  ...
  patterns:
    ...
    - "Rollup externalization: all @streem/* subpaths (including jsx subpaths) must be listed in external array"
...
key-decisions:
  ...
  - "Rollup external array must include @streem/dom/jsx-runtime and @streem/dom/jsx-dev-runtime subpaths separately — the parent package externalization does not cover subpath imports, causing Rollup to bundle the shared h.js chunk"
  - "startBatch/endBatch (from @streem/core) are NOT re-exported — they are internal scheduling primitives; public batch() from @streem/streams is the developer API"
  ...
  - "@streem/lit is NOT included in the meta-package — Lit is an optional peer dep that must remain opt-in"
...
**`streem` meta-package with three-entry Vite build: 666-byte barrel re-exporting all developer primitives from @streem/core, @streem/dom, and @streem/streams — jsxImportSource: 'streem' fully wired**
...
- `packages/streem/vite.config.ts` - Vite library build with three entries, all @streem/* subpaths externalized
- `packages/streem/tsconfig.json` - TypeScript config extending monorepo base
- `packages/streem/src/index.ts` - Barrel: explicit named re-exports from @streem/core, @streem/dom, @streem/streams
- `packages/streem/src/jsx-runtime.ts` - `export * from '@streem/dom/jsx-runtime'`
- `packages/streem/src/jsx-dev-runtime.ts` - `export * from '@streem/dom/jsx-dev-runtime'`

## Decisions Made
- Rollup `external` array must include `@streem/dom/jsx-runtime` and `@streem/dom/jsx-dev-runtime` explicitly — without these, Rollup follows the workspace symlink into the `@streem/dom` source and bundles the `h.js` shared chunk, producing a 3.5KB spurious file alongside the tiny jsx-runtime entry.
...
**1. [Rule 1 - Bug] Added @streem/dom jsx subpaths to Rollup external array**
- **Found during:** Task 2 (build and verify dist)
- **Issue:** vite.config.ts only listed `'@streem/dom'` in external. Rollup resolves `@streem/dom/jsx-runtime` as a subpath import, follows the workspace symlink into `@streem/dom/src/jsx-runtime.ts`, and inlines the internal shared chunk (`h-BDSU-1WM-irMX1593.js`, 3.5KB) into the streem dist. This defeats the "re-exports only" contract.
- **Fix:** Added `'@streem/dom/jsx-runtime'` and `'@streem/dom/jsx-dev-runtime'` to `rollupOptions.external`. Build now produces only 3 files (index.js 666B, jsx-runtime.js 41B, jsx-dev-runtime.js 45B).
...
**Impact on plan:** Fix was necessary for correctness — without it the dist would bundle internal @streem/dom code, violating the "meta-package dist is ~200 bytes (re-exports only, sub-packages not bundled)" must-have truth.
