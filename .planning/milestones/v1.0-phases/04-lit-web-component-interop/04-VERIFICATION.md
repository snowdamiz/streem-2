...
| 7  | `/lit` package exists and builds with zero runtime Lit dependency | VERIFIED | `packages/lit/dist/index.js` imports only from `"/core"` — no `lit` import; `package.json` lists `lit` as devDependency only |
...
| 11 | The `declare module` target is exactly `'/dom/jsx-runtime'` | VERIFIED | `base-custom-element-types.d.ts` line 13: `declare module '/dom/jsx-runtime'`; `src/lit-types/lit-elements.d.ts` line 11: same |
...
| 13 | Developer can run `gen:lit-types` and get `lit-elements.d.ts` augmenting `/dom/jsx-runtime` | VERIFIED | `scripts/gen-lit-types.ts` imports `generateJsxTypes` from `@wc-toolkit/jsx-types`, reads `custom-elements.json`, wraps output in `declare module '/dom/jsx-runtime'`; placeholder `src/lit-types/lit-elements.d.ts` already contains correct module wrapper |
...
|----------|----------|--------|---------|
| `packages/dom/src/h.ts` | Extended `applyProps()` with `prop:/attr:/on:` namespace prefix dispatch | VERIFIED | All 3 branches present; `effect` imported from `/core`; `on:` before `on*` handler |
| `packages/lit/package.json` | Package manifest with workspace peer deps; no Lit runtime dep | VERIFIED | `peerDependencies: { "/core": "workspace:*", "/dom": "workspace:*" }`; `lit` in devDependencies only |
...
| `packages/lit/src/observe-lit-prop.ts` | `observeLitProp` with camelCase-to-kebab-case conversion and `onCleanup` removal | VERIFIED | Imports `signal`, `onCleanup` from `/core`; camelCase regex on lines 51-52; `onCleanup` on line 64 |
| `packages/lit/src/base-custom-element-types.d.ts` | Base JSX types for all hyphenated custom element tags | VERIFIED | Augments `/dom/jsx-runtime`; catch-all with `part/slot/exportparts` and 3 namespace prefix index signatures |
| `packages/lit/src/lit-types/lit-elements.d.ts` | Placeholder CEM-generated element types | VERIFIED | Contains `declare module '/dom/jsx-runtime'` wrapper |
...
| `packages/lit/scripts/gen-lit-types.ts` | CEM pipeline: `cem analyze` -> `generateJsxTypes()` -> `lit-elements.d.ts` | VERIFIED | `generateJsxTypes` imported; post-processes output to add `declare module '/dom/jsx-runtime'` wrapper |
...
| `packages/lit/dist/index.js` | Built `/lit` with no Lit runtime bundled | VERIFIED | Only import: `from "/core"`; Lit is externalized correctly |
...
| `packages/lit/src/observe-lit-prop.ts` | `/core` (`signal`, `onCleanup`) | Workspace peer dependency import | WIRED | Line 1: `import { signal, onCleanup } from '/core'`; externalized in dist |
| `packages/lit/src/base-custom-element-types.d.ts` | `/dom/jsx-runtime` | `declare module` augmentation | WIRED | Line 13: `declare module '/dom/jsx-runtime'` — exact subpath match |
...
| `packages/lit/src/lit-types/lit-elements.d.ts` | `/dom/jsx-runtime` | `declare module` augmentation | WIRED | Line 11: `declare module '/dom/jsx-runtime'` in placeholder file |
...
|-------------|-------------|-------------|--------|----------|
| LIT-01 | 02, 03 | Developer can import and render Lit web components in TSX files with TypeScript-typed props — no runtime wrapper library required | SATISFIED | `/lit` exports `observeLitProp` for reactive pull; `base-custom-element-types.d.ts` provides typed props for hyphenated tags; browser tests in 04-03 prove TypeScript-typed usage works end-to-end |
...
| `packages/lit/vitest.config.ts` | `passWithNoTests: true` | Info | Intentional — no non-browser unit tests in `/lit` yet. Browser tests live in the separate `vitest.browser.config.ts` suite. |
...
| `1a4a0fc` | chore(04-02): scaffold /lit package infrastructure |
...
| Suite | Runner | Result |
|-------|--------|--------|
| `/dom` (93 tests) | Vitest node | 8 files passed, 93 tests passed — zero regressions from applyProps changes |
| `/lit` (node) | Vitest node | `passWithNoTests: true` — exits 0; no non-browser unit tests present (by design) |
| `/lit:browser` (7 tests) | Vitest Browser Mode / Playwright Chromium | 1 file passed, 7 tests passed |
...
- **Plan 01** extended `applyProps()` in `/dom` with `prop:/attr:/on:` prefix dispatch, correctly ordered before the existing `on*` handler.
- **Plan 02** scaffolded `/lit` with `bindLitProp` (effect-based property sync), `observeLitProp` (event-driven signal pull), and `base-custom-element-types.d.ts` (JSX augmentation for all hyphenated custom element tags).
...
The `/dom` test suite (93 tests) passes with zero regressions. The browser test suite (7 tests) passes in Playwright Chromium headless.
