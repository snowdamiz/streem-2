...
**Phase Goal:** Reproducible benchmark suite comparing /core reactive primitives (signal, computed, effect) against SolidJS and Preact signals, with committed results in BENCHMARKS.md.
**Verified:** 2026-02-28
**Status:** passed
...
|-----|----------------------------------------------------------------------------------------------------|------------|----------------------------------------------------------------------------------------|
| 1   | Running `node apps/bench/run.mjs` exits 0 and prints ops/second numbers for all three libraries   | ✓ VERIFIED | run.mjs is 37 lines, imports all three suites, calls printResults with { bench, name }; no stubs; tinybench installed in node_modules; /core dist/index.js present |
| 2   | signal(), computed(), and effect() are each benchmarked for /core, SolidJS, and Preact     | ✓ VERIFIED | Three suite files each add 3 tasks; BENCHMARKS.md contains 9 distinct numeric ops/sec values (grep confirmed) |
...
|-----------------------------------|-------------------------------------------------------------------|------------|------------------------------------------------------------------------------------------------------|
| `apps/bench/package.json`         | tinybench, @preact/signals-core, solid-js, /core workspace deps | ✓ VERIFIED | All four deps present; type: module; bench script: `node run.mjs`                             |
...
| `apps/bench/suites/signal.mjs`    | Signal read/write benchmark for all three libraries               | ✓ VERIFIED | 43 lines; 3 bench.add() tasks: /core, @preact/signals-core, solid-js; returns { bench, name } |
...
| `apps/bench/suites/signal.mjs` | `/core`       | `from '/core'`                | ✓ WIRED   | Line 2: `import { signal, createRoot } from '/core'`; signal and createRoot used in bench tasks               |
| `apps/bench/suites/computed.mjs` | `/core`     | `from '/core'`                | ✓ WIRED   | Line 2: `import { signal, computed, createRoot } from '/core'`; all three used in bench tasks                 |
| `apps/bench/suites/effect.mjs` | `/core`       | `from '/core'`                | ✓ WIRED   | Line 2: `import { signal, effect, createRoot } from '/core'`; all three used in bench tasks                   |
...
1. A runnable benchmark package at `apps/bench/` with tinybench suites covering all three reactive primitives (signal, computed, effect) against all three libraries (/core, @preact/signals-core, solid-js) — 9 benchmark cases total.
