...
- [x] **LIT-01**: Developer gets TypeScript IntrinsicElements autocomplete and type-checking for sl-* elements when /lit is installed (fixes missing JSX type augmentation in dist/)
...
- [x] **TEST-03**: Unit tests cover /dom edge cases: nested ErrorBoundary, Suspense async error propagation, For keyed list reordering
- [x] **TEST-04**: Unit tests cover /streams edge cases: reconnect backoff exhaustion, cancellation during read, subscription error paths
...
- [x] **STYLE-02**: `CSSProperties` type is exported from `/dom` and the `streem` meta-package so style objects can be typed and extracted as variables
